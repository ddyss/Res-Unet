...
'''
将CNN_0、1 最后的32都换成1， total最后卷积是3到1，第一个的channel任意
扩展另一种attention3：直接上来就gru然后attention，不经过前面的CNN_0
'''
# def to_var(x):
...
class CNN_0(nn.Module):
    ...
    def forward(self, x):        # shape: batch x
        ...
        return encoded
class CNN_1(nn.Module): #batch 1 512-----batch 1 128
    def __init__(self):
        ...
        self.norm1 = nn.BatchNorm1d(32) # batch
        self.pool1 = nn.MaxPool1d(2, return_indices=True) # batch

        self.conv2_1 = nn.Conv1d(32, 16, 3,1,1) #
        self.conv2_2 = nn.Conv1d(32, 16, 5,1,2) # batch

        self.norm2 = nn.BatchNorm1d(32)   #batch
        self.pool2 = nn.MaxPool1d(2, return_indices=True) # batch

        self.convf1 = Singleconv1d(32,16)
        self.convf2 = Singleconv1d(16,1)
...
class ChannelNet(nn.Module):
    ...
    def forward(self, x):  # shape: batch x channel x 512 x 128
        encoded = torch.zeros(x.size(0), 1, self.hidden_size, x.size(3))  # batch x channel x hidden_size
...
class TotalNet(nn.Module):
    def __init__(self,in_ch,out_ch):
        ...
        self.conv1 = CNN_0()#out: batch x 1 x 128 x 128   all
        self.convn1 = AsymUNet(1)#out: batch x 1 x 128 x 128  all
        self.conv2 = ChannelNet(128)#out: batch x 1 x 128 x 128  local
        self.gru = RNN(128, 64, 2)
        self.att = Attention(128,128)#out: batch x 2 x 128 x 128
